...
enum PlayerNum : char { None, Player1 = 1, Player2 = 2 };
...
//! plus keeping track of king and rook movement to verify if castling is
//! allowed
...
public:
        ...
        for (int y = height() - 1; y >= 0; --y) {
            ...
            for (int x = 0; x < width(); ++x) {
                ...
                if (c == 0) {
                    ...
                    try {
                        auto g = (player == 2) ? graphics.at(c).first
                                               : graphics.at(c).second;
                        stream << " " + g + " ";
                    }
                    catch (...) {
    ...
    static PlayerNum otherPlayer(PlayerNum player) {
        return (player == PlayerNum::Player1) ? PlayerNum::Player2
                                              : PlayerNum::Player1;
    }
    ...
    bool willMoveThreatOwnKing(BoardTransition transition,
                               PlayerNum player) const {
    ...
    bool isMoveValid(
        int fromX, int fromY, int toX, int toY, PlayerNum player) const {
    ...
    bool canPieceMoveTo(
        ...
        if (fromCell.type == 'p') { // Pawn
            if (toCell.player == otherPlayer(player)) {
                if (abs(fromX - toX) == 1 &&
                    (toY - fromY) == forwardDirection(player)) {
                    return true;
                }
            }
            else {
                if (fromX == toX &&
                    (toY - fromY) * forwardDirection(player) > 0) {
                    if (abs(toY - fromY) == 1) {
                        return true;
                    }
                    else if (abs(toY - fromY) == 2) {
                        if (at(fromX, fromY + forwardDirection(player)).type ==
                            0) {
                            return true;
                        }
                    }
    ...
    std::vector<BoardTransition> getValidMoves(PlayerNum player,
    ...
    bool isCastling(BoardTransition transition) const {
        ...
        auto player =
            (transition.fromY == 0) ? PlayerNum::Player1 : PlayerNum::Player2;
        if (fromCell.type == 'k') {
            ...
            if (transition.fromX == 4 &&
                (transition.fromY == 0 || transition.fromY == 7)) {
    ...
    bool isCastlingValid(BoardTransition transition) const {
        ...
        // Check if
        // 1. The path is free
        // 2. Moving the king does not lead to chess at any point during the
        // move
        if (transition.toX > transition.fromX) {
    ...
    //! If the king or some rook is moved this will affect if castling is
    //! allowed this function keeps track of what is allowed after a move
